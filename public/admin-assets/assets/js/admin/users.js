		...
		$("#users").on("click", ".btn-edit", function () {
			...
			$.ajax({
				...
				success: function (response) {
					...
					editModal.find(".modal-title").text(user.user_name);
					editModal.find(".user_full_name").val(user.user_full_name);
					editModal.find(".user_email").val(user.user_email);
					editModal.find(".user_phone").val(user.user_phone);
					editModal.find(".user_category").val(user.user_category);
					editModal.find(".user_status").val(user.user_status);
					editModal.find(".user_fb_url").val(user.user_fb_url);
					editModal
						.find(".user_instagram_url")
						.val(user.user_instagram_url);
					editModal.find(".user_x_url").val(user.user_x_url);
					editModal
						.find(".user_whatsapp_url")
						.val(user.user_whatsapp_url);
					editModal
						.find(".user_youtube_url")
						.val(user.user_youtube_url);
					editModal
						.find(".user_linkedin_url")
						.val(user.user_linkedin_url);
			...
			columns: [
				...
				},
				{
					...
					render: function (data) {
						var imageURL = `${API_HOST_NAME}/uploads/user-images/${data}`;
						var imageHTML = `<img src="${imageURL}" class="rounded-circle" alt="User Avatar preview" width="40"/>`;
