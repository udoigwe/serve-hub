$(function () {
	...
	function loadSubscriptions() {
		var subscriberID = payloadClaim(token, "user_id");

		blockUI();

		$.ajax({
			type: "GET",
			url: `${API_URL_ROOT}/subscriptions?subscriber_id=${subscriberID}`,
			dataType: "json",
			headers: { "x-access-token": token },
			success: function (response) {
				...
				for (var i = 0; i < subscriptions.length; i++) {
					...
					html += `
                        <tr>
                            ...
                            <td class="text-gray fw-500">${
								subscription.no_of_services
							}</td>
                            <td class="text-gray fw-500">${
								subscription.no_of_images_per_service
							}</td>
				...
				$(".subscriptions").find("tbody").append(html);
				unblockUI();
			},
			error: function (req, status, error) {
				unblockUI();
